...
public class ControllerAdvice {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ExceptionBody handleConstraintValidation(ConstraintViolationException e) {
        ExceptionBody exceptionBody = new ExceptionBody("Validation failed");
        exceptionBody.setErrors(e.getConstraintViolations()
                .stream()
                .collect(Collectors.toMap(
                        validation -> validation.getPropertyPath().toString(),   // ControllerAdvice::apply,
                        validation -> validation.getMessage()                   //ConstraintViolation::getMessage
                )));
        return exceptionBody;
    }
